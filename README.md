...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Classifying genres and decades proved to be a hard task that resulted in low accuracies unless we used a nerual net but that would give us no insight into the artistic features of movie posters. We had to manually extract features that we leanred in class and new features that we read about. For the genre classification we weren't able to get high accuracies and training on all the genres was difficult but we found important features for each genre which we considered very important. For example, we saw that horror movies had lower brightness than other genres which makes sense given they probablty have darker posters. For the decade classifion, our random forest classifier resulted in an accuracy around 30% from 5 decades but we consider this to be okay considering the number of labels and finding features relevant to the decade. 
